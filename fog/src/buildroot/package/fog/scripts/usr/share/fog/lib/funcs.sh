...
customInventory() {
    ...
    tvar=$(lsblk --bytes -dplno SIZE -I 3,8,9,179,259 $mbasset)
    caseman=${tvar: 0:-9}
    casever=$(hdparm -I $mbasset | grep 'Serial Number:')  
    casever1=$(lsblk -dplno SIZE -I 3,8,9,179,259 $mbasset)
    caseserial=$(cat /proc/cpuinfo | grep -m 1 'model name' | head -n1)
    sysversion=$(dmidecode -t 19 | grep 'Range Size: ' | head -n1)
    casesasset=$(dmidecode -t 17 | grep 'Size:' | grep -Ec '[0-9]' | head -n1)
    biosversion=$(dmidecode -t 17 | grep -m 1 'Speed:' | grep -v 'Con' | grep -v 'Unk' | head -n1)
    biosvendor=$(lspci | grep -i 'vga\|3d\|2d' | grep -i 'amd\|nvidia\|radeon' | head -n1)
    biosdate=$(dmidecode -t 4 | grep -i socket | head -n1)
    mbman=$(dmidecode -t 4 | grep -i upgrade | head -n1)
    mbversion=$(dmidecode -t 17 | grep -i 'Type:' | head -n1)
    cpumax=$(lsblk -dplno ROTA $mbasset | sed -e "s/  1/HDD/;s/  0/SSD/")
    hdinfo=$(dmidecode -t 17 | grep -m 1 -i 'Volt' | grep -Ec '[0-9]' | head -n1)
